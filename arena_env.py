"""
ArenaEnv — простая мультиагентная арена с вариативными типами боёв:
1x1, 1x2, 2x1, 2x2, ... (задаётся списками ally_choices/enemy_choices).

Главное:
- Наблюдения entity-centric с масками (ally/enemy mask, enemy_action_mask).
- Действия — четыре головы: target (Discrete), move (Box(2)), aim (Box(2)), fire (Discrete(2)).
- Жёсткая валидация действий (assert_invalid_actions=True) + счётчики invalid_*.
- В infos добавлен 'team_step_reward' — нужен GSPO/GRPO (групповые advantages).
"""
...
class ArenaEnv(MultiAgentEnv):
    ...
    def step(self, action_dict: Dict[str, Dict[str, Any]]):
        ...
        for aid, act in action_dict.items():
            ...
            valid = (0 <= tgt_idx < len(enemy_ids)) and self._is_alive(enemy_ids[tgt_idx])
            if not valid:
                self.count_invalid_target += 1
                if self.assert_invalid:
                    raise AssertionError(f"invalid target: idx={tgt_idx}, alive_ids={enemy_ids}")
        ...
        for aid in list(self._agents_red + self._agents_blue):
            if self._hp[aid] <= 0 and self._is_alive(aid):
                if self._team[aid] == TEAM_RED:  self._alive_red[aid] = False
                else:                             self._alive_blue[aid] = False
    ...
    def _is_alive(self, aid: str) -> bool:
        if aid.startswith("red_"):  return self._alive_red.get(aid, False)
        else:                       return self._alive_blue.get(aid, False)
    ...
    def _build_obs(self, aid: str) -> Dict[str, np.ndarray]:
        ...
        self_vec = np.concatenate([
            self._pos[aid], np.array([self._hp[aid] / 100.0], dtype=np.float32),
